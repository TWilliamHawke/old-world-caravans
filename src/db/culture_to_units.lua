--unit_key = { weight, min_count, max_count }
Old_world_caravans.culture_to_units = {
  wh3_main_sc_cth_cathay = {
    weakA = {
      ["wh3_main_cth_inf_jade_warriors_0"] = { 1, 2, 3 },
      ["wh3_main_cth_inf_jade_warriors_1"] = { 2, 2, 2 },
      ["wh3_main_cth_inf_peasant_spearmen_1"] = { 2, 2, 3 },
      ["wh3_main_cth_cav_peasant_horsemen_0"] = { 1, 2, 2 },
    },
    weakB = {
      ["wh3_main_cth_inf_jade_warrior_crossbowmen_0"] = { 1, 2, 2 },
      ["wh3_main_cth_inf_jade_warrior_crossbowmen_1"] = { 1, 2, 2 },
      ["wh3_main_cth_inf_peasant_archers_0"] = { 2, 2, 3 },
    },
    strongA = {
      ["wh3_main_cth_inf_jade_warriors_1"] = { 3, 1, 2 },
      ["wh3_main_cth_inf_dragon_guard_0"] = { 3, 2, 3 },
      ["wh3_main_cth_art_fire_rain_rocket_battery_0"] = { 1, 1, 1 },
      ["wh3_main_cth_art_grand_cannon_0"] = { 1, 1, 1 },
    },
    strongB = {
      ["wh3_main_cth_cav_jade_lancers_0"] = { 3, 2, 2 },
      ["wh3_main_cth_cav_jade_longma_riders_0"] = { 2, 2, 2 },
      ["wh3_main_cth_veh_sky_lantern_0"] = { 1, 1, 1 },
      ["wh3_main_cth_veh_sky_junk_0"] = { 1, 1, 1 },
    },
  },
  wh_main_sc_emp_empire = {
    weakA = {
      ["wh_main_emp_inf_swordsmen"] = { 2, 2, 3 },
      ["wh_dlc04_emp_inf_flagellants_0"] = { 2, 2, 3 },
      ["wh_main_emp_inf_spearmen_1"] = { 2, 2, 3 },
      ["wh_main_emp_inf_halberdiers"] = { 2, 2, 2 },
      ["wh_main_emp_cav_empire_knights"] = { 2, 2, 2 },
    },
    weakB = {
      ["wh_dlc04_emp_inf_free_company_militia_0"] = { 2, 2, 3 },
      ["wh_main_emp_inf_handgunners"] = { 2, 2, 3 },
      ["wh_main_emp_inf_crossbowmen"] = { 2, 2, 3 },
      ["wh2_dlc13_emp_inf_huntsmen_0"] = { 1, 2, 2 },
    },
    strongA = {
      ["wh_main_emp_inf_greatswords"] = { 3, 1, 2 },
      ["wh_main_emp_inf_halberdiers"] = { 3, 2, 3 },
      ["wh_main_emp_art_helstorm_rocket_battery"] = { 1, 1, 1 },
      ["wh_main_emp_art_helblaster_volley_gun"] = { 1, 1, 1 },
    },
    strongB = {
      ["wh_main_emp_cav_reiksguard"] = { 2, 2, 2 },
      ["wh_main_emp_cav_empire_knights"] = { 3, 2, 2 },
      ["wh2_dlc13_emp_veh_war_wagon_0"] = { 3, 2, 2 },
      ["wh_main_emp_cav_demigryph_knights_0"] = { 1, 1, 1 },
      ["wh_main_emp_cav_demigryph_knights_1"] = { 1, 1, 1 },
    },
  },
  wh_main_sc_brt_bretonnia = {
    weakA = {
      ["wh_main_brt_inf_spearmen_at_arms"] = { 2, 2, 3 },
      ["wh_main_brt_inf_men_at_arms"] = { 2, 2, 3 },
      ["wh_dlc07_brt_inf_men_at_arms_2"] = { 2, 2, 3 },
      ["wh_dlc07_brt_cav_knights_errant_0"] = { 2, 2, 2 },
    },
    weakB = {
      ["wh_dlc07_brt_inf_peasant_bowmen_1"] = { 1, 2, 3 },
      ["wh_dlc07_brt_inf_peasant_bowmen_2"] = { 1, 2, 3 },
    },
    strongA = {
      ["wh_dlc07_brt_inf_battle_pilgrims_0"] = { 1, 2, 3 },
      ["wh_dlc07_brt_inf_foot_squires_0"] = { 1, 2, 3 },
    },
    strongB = {
      ["wh_dlc07_brt_cav_knights_errant_0"] = { 5, 2, 2 },
      ["wh_dlc07_brt_cav_questing_knights_0"] = { 2, 2, 2 },
      ["wh_main_brt_cav_knights_of_the_realm"] = { 2, 2, 2 },
      ["wh_main_brt_cav_pegasus_knights"] = { 1, 1, 1 },
    },
  },
  wh3_main_sc_ksl_kislev = {
    weakA = {
      ["wh3_main_ksl_inf_armoured_kossars_0"] = { 3, 2, 3 },
      ["wh3_main_ksl_cav_horse_raiders_0"] = { 1, 2, 2 },
    },
    weakB = {
      ["wh3_main_ksl_inf_kossars_0"] = { 2, 2, 3 },
      ["wh3_main_ksl_inf_kossars_1"] = { 1, 2, 3 },
      ["wh3_main_ksl_inf_streltsi_0"] = { 1, 2, 2 },
    },
    strongA = {
      ["wh3_main_ksl_inf_tzar_guard_0"] = { 1, 2, 2 },
      ["wh3_main_ksl_inf_tzar_guard_1"] = { 1, 2, 2 },
      ["wh3_main_ksl_inf_ice_guard_0"] = { 1, 2, 2 },
      ["wh3_main_ksl_inf_ice_guard_1"] = { 1, 2, 2 },
    },
    strongB = {
      ["wh3_main_ksl_cav_winged_lancers_0"] = { 1, 2, 2 },
      ["wh3_main_ksl_veh_light_war_sled_0"] = { 1, 2, 2 },
    },
  },
  wh_main_sc_dwf_dwarfs = {
    weakA = {
      ["wh_main_dwf_inf_miners_1"] = { 1, 3, 4 },
      ["wh_main_dwf_inf_dwarf_warrior_0"] = { 3, 2, 3 },
      ["wh_main_dwf_inf_slayers"] = { 1, 2, 2 },
      ["wh_main_dwf_inf_longbeards"] = { 1, 2, 2 },
    },
    weakB = {
      ["wh_main_dwf_inf_quarrellers_0"] = { 1, 2, 3 },
      ["wh_main_dwf_inf_thunderers_0"] = { 1, 2, 3 },
      ["wh_dlc06_dwf_inf_rangers_0"] = { 1, 2, 3 },
    },
    strongA = {
      ["wh_dlc06_dwf_inf_bugmans_rangers_0"] = { 4, 2, 3 },
      ["wh_main_dwf_inf_irondrakes_0"] = { 1, 1, 1 },
      ["wh_main_dwf_inf_irondrakes_2"] = { 1, 1, 1 },
      ["wh_main_dwf_art_organ_gun"] = { 1, 1, 1 },
      ["wh_main_dwf_art_flame_cannon"] = { 1, 1, 1 },
    },
    strongB = {
      ["wh2_dlc10_dwf_inf_giant_slayers"] = { 1, 2, 2 },
      ["wh_main_dwf_inf_longbeards"] = { 4, 2, 2 },
      ["wh_main_dwf_inf_ironbreakers"] = { 1, 1, 1 },
      ["wh_main_dwf_inf_hammerers"] = { 1, 1, 1 },
    },
  },
  wh3_main_sc_ogr_ogre_kingdoms = {
    weakA = {
      ["wh3_main_ogr_inf_ogres_1"] = { 1, 2, 2 },
      ["wh3_main_ogr_inf_ogres_2"] = { 1, 2, 2 },
    },
    weakB = {
      ["wh_main_dwf_inf_quarrellers_0"] = { 1, 2, 3 },
      ["wh_main_dwf_inf_thunderers_0"] = { 1, 2, 3 },
      ["wh_dlc06_dwf_inf_rangers_0"] = { 1, 2, 3 },
    },
    strongA = {
      ["wh3_main_ogr_cav_mournfang_cavalry_0"] = { 1, 2, 2 },
      ["wh3_main_ogr_inf_maneaters_3"] = { 1, 2, 2 },
      ["wh3_main_ogr_inf_leadbelchers_0"] = { 1, 2, 2 },
    },
    strongB = {
      ["wh2_dlc10_dwf_inf_giant_slayers"] = { 1, 2, 2 },
      ["wh_dlc06_dwf_inf_bugmans_rangers_0"] = { 4, 2, 3 },
      ["wh_main_dwf_inf_longbeards"] = { 4, 2, 2 },
      ["wh_main_dwf_inf_ironbreakers"] = { 1, 1, 1 },
      ["wh_main_dwf_inf_hammerers"] = { 1, 1, 1 },
    },
  },
  wh2_main_sc_hef_high_elves = {
    weakA = {
      ["wh2_dlc15_hef_inf_rangers_0"] = { 1, 2, 3 },
      ["wh2_main_hef_inf_spearmen_0"] = { 1, 2, 3 },
    },
    weakB = {
      ["wh2_main_hef_inf_archers_1"] = { 2, 2, 2 },
      ["wh2_main_hef_inf_lothern_sea_guard_0"] = { 1, 2, 3 },
    },
    strongA = {
      ["wh2_dlc15_hef_inf_silverin_guard_0"] = { 2, 2, 2 },
      ["wh2_main_hef_inf_swordmasters_of_hoeth_0"] = { 1, 1, 1 },
      ["wh2_main_hef_inf_white_lions_of_chrace_0"] = { 1, 2, 2 },
    },
    strongB = {
      ["wh2_main_hef_cav_silver_helms_1"] = { 1, 2, 2 },
      ["wh2_main_hef_inf_lothern_sea_guard_1"] = { 2, 2, 3 },
      ["wh2_main_hef_art_eagle_claw_bolt_thrower"] = { 1, 1, 1 },
    },
  },
  wh2_main_sc_lzd_lizardmen = {
    weakA = {
      ["wh_main_emp_inf_swordsmen"] = { 2, 2, 3 },
      ["wh_dlc04_emp_inf_flagellants_0"] = { 1, 2, 3 },
      ["wh_main_emp_inf_spearmen_1"] = { 2, 2, 3 },
      ["wh_main_emp_inf_halberdiers"] = { 2, 2, 2 },
    },
    weakB = {
      ["wh_dlc04_emp_inf_free_company_militia_0"] = { 2, 2, 3 },
      ["wh_main_emp_inf_handgunners"] = { 2, 2, 3 },
      ["wh_main_emp_inf_crossbowmen"] = { 2, 2, 3 },
      ["wh2_dlc13_emp_inf_huntsmen_0"] = { 1, 2, 2 },
    },
    strongA = {
      ["wh_main_emp_inf_greatswords"] = { 3, 1, 2 },
      ["wh_main_emp_inf_halberdiers"] = { 3, 2, 3 },
      ["wh_main_emp_art_helstorm_rocket_battery"] = { 1, 1, 1 },
      ["wh_main_emp_art_helblaster_volley_gun"] = { 1, 1, 1 },
    },
    strongB = {
      ["wh_main_emp_cav_reiksguard"] = { 2, 2, 2 },
      ["wh_main_emp_cav_empire_knights"] = { 3, 2, 2 },
      ["wh2_dlc13_emp_veh_war_wagon_0"] = { 3, 2, 2 },
      ["wh_main_emp_cav_demigryph_knights_0"] = { 1, 1, 1 },
      ["wh_main_emp_cav_demigryph_knights_1"] = { 1, 1, 1 },
    },
  },
  wh_dlc05_sc_wef_wood_elves = {
    weakA = {
      ["wh_dlc05_wef_inf_wardancers_0"] = { 1, 2, 3 },
      ["wh_dlc05_wef_inf_eternal_guard_1"] = { 1, 2, 3 },
      ["wh_dlc05_wef_inf_dryads_0"] = { 1, 2, 3 },
    },
    weakB = {
      ["wh_dlc05_wef_inf_glade_guard_1"] = { 1, 2, 3 },
      ["wh_dlc05_wef_inf_glade_guard_2"] = { 1, 2, 3 },
    },
    strongA = {
      ["wh_dlc05_wef_mon_treekin_0"] = { 1, 1, 2 },
      ["wh2_dlc16_wef_inf_bladesingers_0"] = { 1, 1, 2 },
    },
    strongB = {
      ["wh_dlc05_wef_cav_wild_riders_1"] = { 1, 2, 2 },
      ["wh_dlc05_wef_inf_waywatchers_0"] = { 1, 1, 1 },
    },
  },
  wh_main_sc_teb_teb = {
    weakA = {
      ["wh3_main_ogr_inf_ogres_1"] = { 2, 2, 3 },
      ["wh3_main_ogr_inf_ogres_2"] = { 2, 2, 3 },
      ["wh3_main_ogr_inf_ironguts_0"] = { 1, 2, 2 },
    },
    weakB = {
      ["wh_dlc04_emp_inf_free_company_militia_0"] = { 2, 2, 3 },
      ["wh_main_emp_inf_handgunners"] = { 2, 2, 3 },
      ["wh_main_emp_inf_crossbowmen"] = { 2, 2, 3 },
      ["wh2_dlc13_emp_inf_huntsmen_0"] = { 1, 2, 2 },
    },
    strongA = {
      ["wh3_main_ogr_cav_mournfang_cavalry_0"] = { 1, 2, 2 },
      ["wh3_main_ogr_inf_maneaters_3"] = { 1, 2, 2 },
      ["wh3_main_ogr_inf_leadbelchers_0"] = { 1, 2, 2 },
    },
    strongB = {
      ["wh_main_emp_cav_reiksguard"] = { 2, 2, 2 },
      ["wh_main_emp_cav_empire_knights"] = { 3, 2, 2 },
      ["wh2_dlc13_emp_veh_war_wagon_0"] = { 3, 2, 2 },
      ["wh_main_emp_cav_demigryph_knights_0"] = { 1, 1, 1 },
      ["wh_main_emp_cav_demigryph_knights_1"] = { 1, 1, 1 },
    },
  },
}