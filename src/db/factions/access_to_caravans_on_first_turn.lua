--true requires for confederation check
Old_world_caravans.access_to_caravans_on_first_turn = {
  --southern realms
  mixer_teb_border_princes = true,
  mixer_teb_estalia = true,
  mixer_teb_tilea = true,
  mixer_teb_catrazza = true,
  mixer_teb_colombo = false,
  mixer_teb_bilbali = true,
  mixer_teb_gashnag = true,
  mixer_teb_new_world_colonies = false,
  cr_emp_guests_of_the_raja = false,
  cr_teb_almagora = true,
  cr_teb_bilbali = true,
  cr_teb_central_confederacy = true,
  cr_teb_gualcazar = true,
  cr_teb_luccini = true,
  cr_teb_magritta = true,
  cr_teb_miragliano = true,
  cr_teb_monte_castello = true,
  cr_teb_north_confederacy = true,
  cr_teb_nuja = true,
  cr_teb_order_of_the_blazing_sun = true,
  cr_teb_pavona = true,
  cr_teb_reinos_pobres = true,
  cr_teb_remas = true,
  cr_teb_south_confederacy = true,
  cr_teb_tobaro = true,
  cr_teb_trantio = true,
  cr_teb_verezzo = true,
  cr_teb_vizeaya = true,
  mixer_teb_cataph_luccini = true,
  cr_teb_estebans_caballeros = false,
  cr_teb_pigbarter = true,
  --dwarfs
  wh_main_dwf_dwarfs = true,
  wh_main_dwf_karak_azul = true,
  wh_main_dwf_karak_kadrin = true,
  wh_main_dwf_barak_varr = true,
  wh_main_dwf_karak_hirn = true,
  wh_main_dwf_zhufbar = true,
  wh_main_dwf_karak_ziflin = false,
  wh_main_dwf_karak_izor = false,
  wh3_main_dwf_the_ancestral_throng = false,
  wh2_dlc17_dwf_thorek_ironbrow = false,
  wh_main_dwf_kraka_drak = true,
  wh2_dlc15_dwf_clan_helhein = false,
  wh2_main_dwf_greybeards_prospectors = false,
  wh2_main_dwf_karak_zorn = false,
  wh2_main_dwf_spine_of_sotek_dwarfs = false,
  wh_main_dwf_karak_norn = false,
  wh3_main_dwf_karak_azorn = false,
  cr_dwf_firebeards_excavators = false,
  cr_dwf_bugmans_brewery = true,
  cr_dwf_clan_durazgrund = true,
  wh3_dlc25_dwf_malakai = true,
  --empire
  wh_main_emp_averland = true,
  wh2_dlc13_emp_golden_order = false,
  wh_main_emp_marienburg = true,
  wh_main_emp_hochland = true,
  wh_main_emp_middenland = true,
  wh_main_emp_nordland = true,
  wh_main_emp_ostermark = true,
  wh_main_emp_ostland = true,
  mixer_emp_sylvania = true,
  wh_main_emp_empire = true,
  wh_main_emp_stirland = true,
  wh_main_emp_talabecland = true,
  wh_main_emp_wissenland = true,
  wh3_main_emp_cult_of_sigmar = false,
  wh2_dlc13_emp_the_huntmarshals_expedition = false,
  cr_emp_carroburg = true,
  cr_emp_cult_of_ulric = false,
  cr_emp_sigmarsheim = true,
  --bretonnia
  wh3_main_brt_aquitaine = true,
  wh_main_brt_artois = true,
  wh_main_brt_bastonne = true,
  wh_main_brt_bordeleaux = false,
  wh_main_brt_bretonnia = true,
  wh_main_brt_carcassonne = true,
  wh_main_brt_lyonesse = true,
  wh_main_brt_parravon = true,
  wh2_dlc14_brt_chevaliers_de_lyonesse = false,
  cr_brt_leofrics_fellowship = false,
  wh2_main_brt_thegans_crusaders = false,    --is false to prevent Rapance
  wh2_main_brt_knights_of_the_flame = false, --to unlock caravans
  wh2_main_brt_knights_of_origo = false,     --after confederation with them
  mixer_brt_brionne = true,
  mixer_brt_gisoreux = true,
  mixer_brt_languille = true,
  mixer_brt_montfort = true,
  mixer_brt_quenelles = true,
  wh3_dlc20_brt_march_of_couronne = true,
  --kislev
  wh3_main_ksl_the_ice_court = true,
  wh3_main_ksl_brotherhood_of_the_bear = true,
  wh3_main_ksl_druzhina_enclave = true,
  wh3_main_ksl_ropsmenn_clan = true,
  wh3_main_ksl_the_great_orthodoxy = true,
  wh3_main_ksl_ungol_kindred = true,
  wh3_main_ksl_ursun_revivalists = false,
  cr_ksl_heldegrad = false,
  cr_ksl_order_of_the_winter_sun = false,
  cr_ksl_peasant_council = true,
  cr_ksl_the_boyar_assembly = true,
  cr_ksl_tzaryovshchina = true,
}
