Old_world_caravans.node_culture_to_event_weight = {
  wh_main_sc_emp_empire = {
    wh_main_sc_emp_empire = 12,
    wh_main_sc_brt_bretonnia = 30,
    wh3_main_sc_ksl_kislev = 75,
    wh_main_sc_dwf_dwarfs = 75,
    wh3_main_sc_ogr_ogre_kingdoms = 200,
    wh_main_sc_teb_teb = 50,
    wh2_main_sc_hef_high_elves = 45,
    wh_dlc05_sc_wef_wood_elves = 60,
    wh2_main_sc_lzd_lizardmen = 200
  },
  wh_main_sc_brt_bretonnia = {
    wh_main_sc_emp_empire = 15,
    wh_main_sc_brt_bretonnia = 20,
    wh3_main_sc_ksl_kislev = 125,
    wh_main_sc_dwf_dwarfs = 70,
    wh_main_sc_teb_teb = 40,
    wh2_main_sc_hef_high_elves = 50,
    wh_dlc05_sc_wef_wood_elves = 200,
    wh2_main_sc_lzd_lizardmen = 200
  },
  wh_main_sc_dwf_dwarfs = {
    wh_main_sc_dwf_dwarfs = 8,
    wh_main_sc_teb_teb = 150,
    wh3_main_sc_ogr_ogre_kingdoms = 65,
    wh_main_sc_emp_empire = 35,
    wh3_main_sc_ksl_kislev = 95,
    wh2_dlc09_sc_tmb_tomb_kings = 150,
    wh_main_sc_brt_bretonnia = 150
  },
  wh3_main_sc_cth_cathay = {
    wh_main_sc_emp_empire = 24,
    wh3_main_sc_cth_cathay = 99,
    wh3_main_sc_ogr_ogre_kingdoms = 35,
    wh3_main_sc_ksl_kislev = 40,
    wh_main_sc_dwf_dwarfs = 30,
    wh_main_sc_teb_teb = 100,
    wh_main_sc_brt_bretonnia = 200,
    wh_dlc05_sc_wef_wood_elves = 200,
    wh2_main_sc_hef_high_elves = 200,
  },
  mixer_teb_southern_realms = {
    wh_main_sc_emp_empire = 20,
    wh3_main_sc_cth_cathay = 200,
    wh3_main_sc_ogr_ogre_kingdoms = 125,
    wh3_main_sc_ksl_kislev = 200,
    wh_main_sc_dwf_dwarfs = 50,
    wh_main_sc_teb_teb = 24,
    wh_main_sc_brt_bretonnia = 20,
    wh_dlc05_sc_wef_wood_elves = 200,
    wh2_main_sc_hef_high_elves = 40,
    wh2_main_sc_lzd_lizardmen = 200
  },
  wh3_main_sc_ksl_kislev = {
    wh3_main_sc_ksl_kislev = 50,
    wh_main_sc_emp_empire = 12,
    wh_main_sc_brt_bretonnia = 50,
    wh2_main_sc_hef_high_elves = 100,
    wh_main_sc_teb_teb = 70,
    wh_main_sc_dwf_dwarfs = 200,
    wh3_main_sc_ogr_ogre_kingdoms = 50,
    wh3_main_sc_cth_cathay = 40,
    wh_dlc05_sc_wef_wood_elves = 200,
    wh2_main_sc_lzd_lizardmen = 200
  }
}
